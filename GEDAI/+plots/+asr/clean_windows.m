function [signal,sample_mask, Meta] = clean_windows(signal,max_bad_channels,zthresholds,window_len,window_overlap,max_dropout_fraction,min_clean_fraction,truncate_quant,step_sizes,shape_range)
% Sven: 
% Function from clean_rawdata EEGLAB plugin.
% Only added Meta output for plotting purposes
%
...
for c = C:-1:1
    ...
    [mu,sig] = plots.asr.fit_eeg_distribution(X, ...
...
Meta.zRMS = wz;
Meta.Windowlen = window_len;
Meta.Overlap = window_overlap;
Meta.nWindows = numel(offsets);
Meta.zThresholds = zthresholds;
