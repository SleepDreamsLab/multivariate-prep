...
%                 warning(['Chunk ' num2str(iChunk) ': Detected flat channels in chunk were ignored and are added back in after Zapline plus processing: ' num2str(flat_channels_idx_chunk)])
        ...
        % compute spectra
        % LOCAL PATCH: on the first noise frequency, data is still exactly what the
        % initial spectrum was computed from - data is only replaced by cleanData at the
        % end of each while-iteration (see 'data = cleanData' below), and the retry loop
        % leaves it alone. Recomputing it is a full-night, all-channel pwelch for a
